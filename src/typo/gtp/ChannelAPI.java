...
public abstract class ChannelAPI {
	...
	public abstract void onError( Integer errorCode , String description );

    protected abstract String createChannel() throws IOException;
     ...
     */
    public ChannelAPI(String URL) throws IOException {
    	BASE_URL = URL;
    ...
    private static String getData( String pUrl ) throws IOException {
                ...
                while (readyState.equals(ReadyState.OPEN)) {
                    if (parser == null) {
                        if(parserRepolls<3) {
                            ...
                            onError(500, "Parser poll failed "+String.valueOf(parserRepolls)+" times in a row!");
                            return;
                        }
                    } else {
                        parserRepolls=0;
    ...
    private static class TalkMessageParser {
        ...
        public TalkMessageParser(InputStream pInput) throws ChannelException {
            try {
                mReader = new BufferedReader(new InputStreamReader(pInput));
            } catch (IllegalStateException e) {
                throw new ChannelException(e);
            }
        }
    ...
    private static class TalkMessage {
        ...
        private static ArrayList<TalkMessageEntry> parseMessage(BufferedReader reader)
            ...
            ArrayList<TalkMessageEntry> entries = new ArrayList<>();
    ...
    private void connect(XHR xhr) {
        if (xhr.isSuccess()) {
            ...
            readyState = ReadyState.CLOSING;
            onError(xhr.getStatus(), xhr.getStatusText());
            readyState = ReadyState.CLOSED;
            onClose();
        }
    }
